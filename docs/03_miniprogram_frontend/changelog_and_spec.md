...
| `pages/healthcare/index.js` | 1. `initData` 阶段加入针对低版本疫苗数据的唯一 ID 补全及因去重裁剪数据的自动恢复。<br>2. 就诊提醒支持倒计时 `daysLeft` 属性计算。<br>3. 门诊就诊、儿保轨迹、发育评估记录在加载/新增后自动按日期 and `id` 降序排列。 |
| `pages/family/index` | **优化与升级**：<br>1. 重构 `.wxml` 及 `.wxss`，支持在守护成员列表中渲染看护人的微信头像、微信昵称、和角色身份标签，并对历史数据进行自动兜底兼容。<br>2. 新增 “🖼️ 二维码” 按钮，点击在半透明弹窗中以公用 API 渲染同步码二维码，并支持长按保存分享。<br>3. 新增 “📷 扫码绑定 (推荐)” 按钮，调起相机扫描二维码自动填充并绑定家庭组。 |
| `cloudfunctions/updateFamily` | **优化**：在 `create`、`addMember` 以及 `update` 操作中维护成员头像和昵称明细数组 `members_info`，且在用户非管理员更新头像时自动防范对创建者字段的越权覆写。 |
| `pages/sleep/index` | **睡眠云同步优化（2026-07-01）**：<br>1. 开启配置中下拉刷新功能并编写 `onPullDownRefresh` 生命周期。<br>2. 接入 `syncPull('sleep_records')` 云同步，在 `onShow` 和下拉刷新时增量静默更新，实现多设备睡眠记录实时同步。 |
| `utils/storage.js` | **睡眠集合适配（2026-07-01）**：在 `triggerAutoCloudSync`、`syncPull` 和 `syncMerge` 中添加 `'baby_sleep_records': 'sleep_records'` 映射，实现睡眠记录本地缓存到云数据库的双向自动同步。 |
| `cloudfunctions/login` | **集合扩展（2026-07-01）**：在 `BUSINESS_COLLECTIONS` 中追加 `'sleep_records'`，使核心云函数能正常查询睡眠记录表。 |
| `cloudfunctions/updateFamily` | **管控增强（2026-07-01）**：<br>1. 升级 `removeMember` 支持通过 `targetOpenid` 移除指定协同成员（仅创建者权限）。<br>2. 新增 `dismissFamily` 解散家庭组动作，物理删除云端家庭记录。 |
| `pages/family/index.js` | **权限与踢出拦截（2026-07-01）**：<br>1. 改以云端 `members` 为基准循环，匹配 `members_info` 修复创建者缺席，强制置顶并智能降级他人本地头像路径。<br>2. 实现 `removeMember` 单向移除成员，`leaveFamily` 分流创建者“注销”与协同者“解绑退出”逻辑（创建者解散受协同人未空限制，退组接入 removeMember 云同步）。<br>3. 拦截未绑定状态：如本地存有家庭 ID 但云端拉取为空，自动清空本地并弹出“断开”提示。 |
| `utils/storage.js` | **静默踢出检测（2026-07-01）**：在 `syncPull` 云同步中，如本地存有家庭 ID 但云端 `login` 无匹配家庭（已被踢出或解散），自动物理清空本地 ID 并弹窗拦截提示，自动刷新清空各业务页面共享数据。 |

| `pages/my/index.js` | **头像保存优化（2026-07-01）**：修改 `saveEditProfile` 头像保存函数中的 `finish` 回调，优先储存 `cloudUrl` 云路径文件 ID 而非本地路径，保证他人加载时路径全局可用。 |
