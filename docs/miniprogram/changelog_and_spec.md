...
| `cloudfunctions/updateFamily` | **解绑机制扩展（2026-07-01）**：新增 `action === 'removeMember'` 支持物理在云端 `members` 和 `members_info` 数组中移除退出成员的 OpenID。 |
| `pages/family/index.js` | **数据映射与退组改造（2026-07-01）**：<br>1. 改以云端 `members` 数组为循环基准，智能适配 `members_info`，修复创建者缺席的问题。<br>2. 改造 `leaveFamily` 接入 `removeMember` 云端解绑动作，彻底解决解绑退组残留问题。<br>3. **排序与纠错**：强制将家庭创建者排在第一位展示；自动屏蔽过滤他人设备的本地路径，降级为默认头像，彻底解决头像空白白圈的问题。 |
