...
| 9 | `growth` | [growth.json](db_seeds/growth.json) | 宝宝生长发育记录（7 条） |
| 10| `classes` | [classes.json](db_seeds/classes.json) | 机构上课考勤与充值记录 |
| 11| `class_customers` | - | 手动创建的课程机构列表 |
...
### 方案一：云开发控制台直接导入 JSON 文件

微信云开发控制台提供了可视化的数据导入功能。**需要特别注意的是，云开发控制台不支持直接导入标准的 JSON 数组格式（即以 `[...]` 包裹、且对象之间用逗号分隔的文件），而是要求导入换行分隔的 JSON 对象（JSON Lines / NDJSON 格式）。**

#### 格式转换示例
如果使用云开发控制台直接导入，需要先将标准 JSON 数组文件转换为以下格式（去掉最外层的 `[]` 和对象之间的逗号，每行为一个完整的 JSON 对象）：
```json
{"id": 1, "date": "2026-01-17", "height": 71.0, "weight": 7.33, "head": 38.7, "foot": null, "note": ""}
{"id": 2, "date": "2026-02-08", "height": 71.0, "weight": 7.365, "head": 39, "foot": null, "note": ""}
```
...
2. 进入 **"数据库"** 标签页。
3. 点击左侧的 `+` 号，依次创建上表中列出的 **11 个集合**（名称必须与表中 `集合名称` 完全一致）。
4. 选中某个集合（例如 `growth`），点击右侧面板上方的 **"导入"** 按钮。
5. 选择转换好格式后的 JSON 文件，冲突处理选择 **"插入"（Insert）**，点击确定。
6. 对其余集合重复上述步骤。
